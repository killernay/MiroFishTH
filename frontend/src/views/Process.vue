      ...
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        ...
        <div class="graph-container" ref="graphContainer">
          ...
          <div v-else-if="currentPhase < 1" class="graph-waiting">
            ...
            <p class="waiting-hint">{{ $t('process.startGraphBuildAfterOntology') }}</p>
          </div>
          ...
          <div v-else-if="currentPhase === 1 && !graphData" class="graph-waiting">
            ...
            <p class="waiting-hint">{{ $t('process.dataWillAppear') }}</p>
          </div>
        ...
        <div class="process-content">
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 0, 'completed': currentPhase > 0 }">
            ...
            <div class="phase-detail">
              ...
              <div class="detail-section" v-if="projectData?.ontology">
                <div class="detail-label">{{ $t('process.generatedRelationshipTypes', { count: projectData.ontology.relation_types?.length || 0 }) }}</div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 2, 'completed': currentPhase > 2 }">
            <div class="phase-header">
              ...
              <div class="phase-info">
                <div class="phase-title">{{ $t('process.buildFinished') }}</div>
                <div class="phase-api">{{ $t('process.nextStepReady') }}</div>
              </div>
...
const statusText = computed(() => {
  ...
  if (currentPhase.value >= 2) return t('process.buildFinished')
...
const goToNextStep = () => {
  // TODO: 进入环境搭建步骤
  alert(t('process.environmentSetupComingSoon'))
}
...
const handleNewProject = async () => {
  ...
  if (!pending.isPending || pending.files.length === 0) {
    error.value = t('process.pendingUploadMissing')
    loading.value = false
    return
  }
  ...
  try {
    ...
    error.value = t('process.projectInitializationFailed', { error: err.message || t('process.unknownError') })
  } finally {
    loading.value = false
  }
}
...
const loadProject = async () => {
  try {
    ...
    if (response.success) {
      ...
      error.value = response.error || t('process.projectLoadFailed', { error: t('process.unknownError') })
    }
  } catch (err) {
    console.error('Load project error:', err)
    error.value = t('process.projectLoadFailed', { error: err.message || t('process.unknownError') })
  } finally {
    loading.value = false
  }
}
...
const startBuildGraph = async () => {
  try {
    ...
    if (response.success) {
      ...
      error.value = response.error || t('process.startGraphBuildFailed', { error: t('process.unknownError') })
      buildProgress.value = null
    }
  } catch (err) {
  ...
  try {
    ...
    if (response.success) {
      ...
      if (task.status === 'completed') {
        console.log('Graph build completed; loading full graph data...')
        ...
        buildProgress.value = {
          progress: 100,
          message: t('process.graphBuildLoading')
        }
        ...
        stopPolling()
        stopGraphPolling()
        error.value = t('process.graphBuildFailedWithError', { error: task.error || t('process.unknownError') })
        buildProgress.value = null
      }
    }
  } catch (err) {
